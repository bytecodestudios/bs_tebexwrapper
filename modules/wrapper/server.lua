...
lib.callback.register('bs_tebexwrapper:server:purchase', function(source, data)
    if not data then return { success = false, message = "Invalid cart data received." } end
        ...
        elseif cartItem.type == 'vehicle' then
            ...
            if vehicleData then
                for i = 1, cartItem.quantity do
                    ...
                    local finalModsJson = json.encode({})
                    local vehicleValues = {
...
lib.callback.register('bs_tebexwrapper:server:redeemCode', function(source, data)
    if not data then return { success = false, message = "Invalid data received." } end

    local Player =  GetPlayer(source)
    if not Player then return { success = false, message = "Player data could not be found." } end

    local license = Player.PlayerData.license
    local codeToRedeem = data.code
    if not codeToRedeem or codeToRedeem:gsub("%s", "") == "" then return { success = false, message = "Please enter a code." } end

    local codeData = MySQL.Sync.fetchAll('SELECT * FROM codes WHERE UPPER(code) = UPPER(@code)', { ['@code'] = codeToRedeem })
    if not codeData or not codeData[1] then return { success = false, message = "This code is invalid or has already been used." } end
    ...
    if totalDiamondsToGive <= 0 then return { success = false, message = "The items in this code are no longer available. Please contact support." } end

    MySQL.Sync.execute('INSERT INTO player_diamonds (license, diamonds) VALUES (@license, @amount) ON DUPLICATE KEY UPDATE diamonds = diamonds + @amount', { ['@license'] = license, ['@amount'] = totalDiamondsToGive })
    ...
    local newBalance = Citizen.Await(getPlayerDiamonds(license))
    return { success = true, message = string.format("Success! You have redeemed %d Diamonds.", totalDiamondsToGive), newBalance = newBalance }
end)
...
---Tebex Webhook Command
RegisterCommand('package_purchase', function(source, args)
	if source == 0 then
        CreateThread(function()
            if not args or not args[1] then print('^1[bs_tebexwrapper] Error: purchase_package_tebex was called with no arguments.^0'); return end
            local success, dec = pcall(json.decode, args[1])
            if not success or not dec then print('^1[bs_tebexwrapper] Error: Could not decode Tebex JSON.^0'); return end
            local tbxid = dec.transactionId or dec.transid
            local packageName = dec.packageName or dec.packagename
            if not tbxid or not packageName then print('^1[bs_tebexwrapper] Error: Tebex JSON missing transactionId or packageName.^0'); return end
